...
shmcomm = MPI.Comm_split_type(world, MPI.COMM_TYPE_SHARED, world_rank)
local_rank = MPI.Comm_rank(shmcomm)

if CUDA.functional()
    ngpu = length(CUDA.devices())
    CUDA.device!(local_rank % ngpu)
    @info "Process $world_rank using GPU $(CUDA.device())"
end

MPI_X = 4
MPI_Y = 2
MPI_Z = 2
@assert MPI_X * MPI_Y * MPI_Z == MPI.Comm_size(world)
