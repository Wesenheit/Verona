...
const thread_rngs = [MersenneTwister(seed + i) for i in 1:Threads.nthreads()]
start_calc = time()

Threads.@threads for num in 1:(P.size_X * P.size_Y * P.size_Z)
    ...
    @fastmath begin 
        ...
        if R < R_max
            if R < R_eng
                rho = Rho0 * (R_max/R_eng)^2 * (1+randn(thread_rngs[Threads.threadid()])*3e-2)
            else
...
Verona3D.HARM_HLL(comm,CuP,MPI_X,MPI_Y,MPI_Z,SizeX,SizeY,SizeZ,dt,dx,dy,dz,T,eos,drops,floor,ARGS[1],n_it,tol,TurnOff,reconstruction_method)
