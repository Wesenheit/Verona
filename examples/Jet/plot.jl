...
hm1 = image!(ax1,Y,X,transpose(log10.(data["data"][1,:,:])), colorrange = (min_val, max_val), colormap = :cork)
hm2 = image!(ax2,Y,X,transpose(log10.(gamma)), colorrange = (0, gam_max), colormap = :hot)
...
record(fig1, "Jet_MPI_CUDA_1.mp4", 1:num; framerate = 3) do i
    ...
    hm1[3] = transpose(log10.(data["data"][1,:,:]))
