...
function SaveHDF5Gather(comm,P::VeronaArr{T},XMPI::Int64,YMPI::Int64,name::String,to_save::Dict) where T
    size = MPI.Comm_size(comm)
    flat = vec(Array{T}( @view P.arr[:,4:end-3,4:end-3]))
    ...
    if MPI.Comm_rank(comm) == 0
        global_matrix = zeros(T,4,XMPI * (P.size_X - 6),YMPI * (P.size_Y - 6))
        for p in 0:(size-1)
            ...
            global_matrix[:, start_x:start_x+(P.size_X - 6)-1, start_y:start_y+(P.size_Y - 6)-1] = 
                        reshape(recvbuf[local_start:local_end], 4, P.size_X-6, P.size_Y-6)
        end
        file = h5open(name,"w")
