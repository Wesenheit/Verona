...
@inline function cmin_cmax_MHD(PL::AbstractVector{T}, PR::AbstractVector{T}, direction:::Int64, eos::Polytrope{T}) where T<:Real
    λ₊_L = λ₋_L = λ₊_R = λ₋_R = 0.0
    ...
    for iter in 1:2
		...
		# Speed of sounds
		cₛ²  = eos.gamma*(u*(eos.gamma-1))/(ρ + eos.gamma*u)
		vₐ²  = bsq/(bsq + ρ + eos.gamma*u)
		cₘₛ² = (cₛ² + vₐ²*(1-cₛ²))/(1+vₐ²)
		...
		if iter == 1  
				λ₊_L = (v + sqrt(cₘₛ²))/(1 + v*sqrt(cₘₛ²))
				λ₋_L = (v - sqrt(cₘₛ²))/(1 - v*sqrt(cₘₛ²))
		else
				λ₊_R = (v + sqrt(cₘₛ²))/(1 + v*sqrt(cₘₛ²))
				λ₋_R = (v - sqrt(cₘₛ²))/(1 - v*sqrt(cₘₛ²))	
		end
	end

    C_max_X =  max(λ₊_L, λ₊_R, 0)
    C_min_X = -min(λ₋_L, λ₋_R,0)
