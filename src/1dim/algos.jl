...
function LaxFriedrich(P::ParVector1D,N::Int64,dt::Float64,dx::Float64,T::Float64,eos::EOS,drops::Float64,kwargs...)
    ...
    while t < T
        ...
        for i in 2:N-1
            @inbounds Ubuffer.arr1[i] =  0.5 * (U.arr1[i - 1] + U.arr1[i + 1]) - 0.5 * dt/dx * (F.arr1[i + 1] - F.arr1[i - 1]) 
            @inbounds Ubuffer.arr2[i] =  0.5 * (U.arr2[i - 1] + U.arr2[i + 1]) - 0.5 * dt/dx * (F.arr2[i + 1] - F.arr2[i - 1]) 
            @inbounds Ubuffer.arr3[i] =  0.5 * (U.arr3[i - 1] + U.arr3[i + 1]) - 0.5 * dt/dx * (F.arr3[i + 1] - F.arr3[i - 1]) 
        end
...
function HARM_HLL(P::ParVector1D,N::Int64,dt::Float64,dx::Float64,T::Float64,eos::EOS,drops::Float64,rec::Symbol,kwargs...)
    ...
    while t < T
        ...
        elseif rec == :wenoz
            @threads :static for i in 3:N-2 # interpolating left and right
                PR.arr1[i-1],PL.arr1[i] = WENOZ(P.arr1[i-2],P.arr1[i-1],P.arr1[i],P.arr1[i+1],P.arr1[i+2])
                PR.arr2[i-1],PL.arr2[i] = WENOZ(P.arr2[i-2],P.arr2[i-1],P.arr2[i],P.arr2[i+1],P.arr2[i+2])
                PR.arr3[i-1],PL.arr3[i] = WENOZ(P.arr3[i-2],P.arr3[i-1],P.arr3[i],P.arr3[i+1],P.arr3[i+2])
            end
        else
        ...
        if rec == :minmod
            @threads :static for i in 2:N-1 # interpolating left and right
                PR.arr1[i-1],PL.arr1[i] = MINMOD(Phalf.arr1[i-1],Phalf.arr1[i],Phalf.arr1[i+1])
                PR.arr2[i-1],PL.arr2[i] = MINMOD(Phalf.arr2[i-1],Phalf.arr2[i],Phalf.arr2[i+1])
                PR.arr3[i-1],PL.arr3[i] = MINMOD(Phalf.arr3[i-1],Phalf.arr3[i],Phalf.arr3[i+1])
            end
        elseif rec == :ppm
            @threads :static for i in 3:N-2 # interpolating left and right
                PR.arr1[i-1],PL.arr1[i] = PPM(Phalf.arr1[i-2],Phalf.arr1[i-1],Phalf.arr1[i],Phalf.arr1[i+1],Phalf.arr1[i+2])
                PR.arr2[i-1],PL.arr2[i] = PPM(Phalf.arr2[i-2],Phalf.arr2[i-1],Phalf.arr2[i],Phalf.arr2[i+1],Phalf.arr2[i+2])
                PR.arr3[i-1],PL.arr3[i] = PPM(Phalf.arr3[i-2],Phalf.arr3[i-1],Phalf.arr3[i],Phalf.arr3[i+1],Phalf.arr3[i+2])
            end
        elseif rec == :wenoz
            @threads :static for i in 3:N-2 # interpolating left and right
                PR.arr1[i-1],PL.arr1[i] = WENOZ(Phalf.arr1[i-2],Phalf.arr1[i-1],Phalf.arr1[i],Phalf.arr1[i+1],Phalf.arr1[i+2])
                PR.arr2[i-1],PL.arr2[i] = WENOZ(Phalf.arr2[i-2],Phalf.arr2[i-1],Phalf.arr2[i],Phalf.arr2[i+1],Phalf.arr2[i+2])
                PR.arr3[i-1],PL.arr3[i] = WENOZ(Phalf.arr3[i-2],Phalf.arr3[i-1],Phalf.arr3[i],Phalf.arr3[i+1],Phalf.arr3[i+2])
            end
        else
            println("reconstruction not supported!")
        end
